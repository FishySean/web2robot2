...
>
> **2026-08-23 追加 B14 / B15（视觉合成第二块"按片段相机渲机器人"撞出来的两条）**：
> B14 是**已有代码里的一处左右手判定矛盾**（改它会让已建立的掩码字节基线失效，所以
> 我只量了、没改）；B15 是**碰撞过滤把手腕推走 5 cm、画面上看得见的错位**（要不要为了
> 画面好看而放松过滤，是取舍不是实现细节）。明细见 §B14 / §B15。
...
| B14 | 手部掩码认"第 0 个槽位就是左手"，但官方 `hand_meta.json::is_right_per_frame` 说有 34 帧不是（`(True, None)` / `(True, True)`）。改了就等于承认现有 `masks.npz` 的 `left`/`right` 两个数组名在这些帧上是错的，已建立的掩码字节基线全失效 | **待拍板**（2026-08-23 提出） | 见下方 §B14。**union（抠人用的那个）不受影响**，所以合成这条主线不卡；只量了没改 |
| B15 | 碰撞过滤（`--arm_torso_collision` / `--dual_hand_collision`）在 IK 已经解到的帧上仍把手腕推走约 5 cm，投到画面上是 20–32 px 的可见错位（16 只手里 2 只） | **待拍板**（2026-08-23 提出） | 见下方 §B15。"为了画面对齐而放松过滤"会把穿躯帧放回来，是取舍不是实现细节，我不自己定 |
...
### B14. 「第 0 个槽位是左手」这个假设有 34 帧不成立（2026-08-23，待拍板）

做视觉合成第二块（按片段相机渲机器人）时，为了把"机器人的左手"和"人的左手"对上，
去读了官方 `hand_meta.json` 的 `is_right_per_frame`，才发现**它和我们掩码代码里的假设
对不上**。
...
| 谁 | 怎么判左右手 |
|---|---|
| `src/web2robot/synth/handmask.py:238` | `idx = {"both": (0, 1), "left": (0,), "right": (1,)}[hands]` —— **槽位 0 写死是左手** |
| `src/web2robot/synth/render.py::hand_slot_sides` | 逐帧读 `is_right_per_frame`，`True` → 右手、`False` → 左手、`None` → 这帧没这只手 |

**实测（`data/clips_official/` 全 10 段，逐帧统计 `is_right_per_frame` 的行取值）**：

| 行取值 | 含义 | 出现在哪 |
|---|---|---|
| `(False, True)` | 槽 0 左、槽 1 右 —— 和 handmask 的假设一致 | 绝大多数帧 |
| `(True, None)` | **只检出一只手，而且是右手，存在槽 0** | `-0RheyDV3a0_474.8_487.3` 27 帧（第 145 帧起）、`-1r9yl-P-Ao_231.8_241.5` 2 帧（第 0–1 帧） |
| `(True, True)` | **两个槽位都是右手**（官方检测重复了） | `-1r9yl-P-Ao_60.4_68.4` 5 帧（第 115–119 帧） |
| `(None, None)` | 两只手都没检出 | `-0RheyDV3a0_474.8_487.3` 17 帧 |

合计 **34 帧**（29 帧是右手被当成左手，5 帧是"左手"其实是第二个右手检测），分布在
10 段里的 3 段。

**影响范围（量过，不是推测）**：

- `masks.npz` 里那两个数组的**名字**在这 34 帧上是错的：`left` 里装的是右手，`right` 是空的。
- `joints_inside_fraction()` 报的 `left` / `right` 两个 `fraction` 同样在这些帧上错位
  （`overall_fraction`、`tip` / `non_tip` 不受影响，因为它们不分左右）。
- **`hands="both"` 的并集不受影响** —— 两个槽位无条件都画。抠人换机器人用的就是并集，
  所以**视觉合成这条主线不卡在这条上**，这也是我没停下来等拍板的原因。

**为什么我没直接改**：改 `handmask.py` 会改掉 `masks.npz` 的字节，
`docs/VERIFICATION.md` 里那 10 段掩码的 md5 基线、以及
`scripts/dev/check_synth_render_bytes.sh` 刚刚建立的"逐字节一致"结论全部失效。
按规矩（「发现但没敢改的存疑数值/设计，记录进 BACKLOG，不要自行决定改动」）记在这。
`tests/test_synth_render.py::TestRealClipInputs` 里有一个测试**专门把"确实存在槽 0 是
右手的片段"钉住**，免得以后有人把这条当成不可能发生的情况优化掉。

**要动的时候动什么**：`hand_slot_sides()` 已经是现成的判据来源，`handmask.py` 里把
`idx` 那一行换成"按帧查 `is_right_per_frame`"就够，改动很小；真正要决定的是
**`(True, True)` 那 5 帧怎么办** —— 两只右手，按名字存哪个槽都不对，
要么丢帧、要么留一只（哪一只？），这是数据取舍。

### B15. 碰撞过滤把手腕推走 5 cm，画面上看得见（2026-08-23，待拍板）

做视觉合成第二块时量出来的：8 段官方片段 16 只手，机器人手腕投影 ↔ 官方 3D 手腕投影
（`robot_mano_px`）中位 1.72 px，但有 3 只在 14–32 px。**归因做完了，是碰撞过滤。**

**怎么坐实的（不是推测）**：同 seed、同 `--root_solver grid`、**只去掉
`--arm_torso_collision --dual_hand_collision`** 重跑两段。grid 不含随机先验，所以
`root_frames.npz` 两份逐位相同（脚本先核对这一条，不一致就报错不出数），于是碰撞过滤
是唯一变量：

| 片段 | 手 | 开过滤 | 关过滤 | 两份轨迹的手腕 3D 位移（中位 / 最大 / 改动帧数） |
|---|---|---|---|---|
| `-1r9yl-P-Ao_86.3_90.8` | 右 | **22.67 px / 5.19 cm** | 1.02 px / 0.22 cm | 4.75 cm / 7.23 cm / 53 帧（共 69） |
| `-1r9yl-P-Ao_86.3_90.8` | 左 | 1.64 px / 0.31 cm | 1.21 px / 0.20 cm | 0.00 cm / 4.90 cm / 31 帧 |
| `-2cNMO9Mm3Q_192.4_209.2` | 左 | **31.56 px / 5.86 cm** | 0.58 px / 0.15 cm | 5.99 cm / 12.82 cm / 103 帧（共 146） |
| `-2cNMO9Mm3Q_192.4_209.2` | 右 | **14.16 px / 2.71 cm** | 0.68 px / 0.21 cm | 2.76 cm / 9.32 cm / 96 帧 |

两段的 `metrics.npz` 都是 `ik_rate` ≈ 1.0、`pos_err` ≤ 0.01 m，也就是 **IK 明明解到了
1 cm 以内，是解完之后的过滤器把手推走的**。复现命令和脚本：
`scripts/dev/attrib_wrist_offset.py`（产物在 `outputs/dev/nocoll/<片段>_grid/`）。

**这不是 bug，是过滤器在干它该干的事** —— 那两段就是手臂贴身的姿态，过滤器为了不穿躯干
必须把手臂往外推。A1 那条已经量过：过滤让 13 段的穿模帧占比从 28.9% 降到 13.3%。
所以这里是**两个都想要但互斥的目标**：

| 要哪个 | 代价 |
|---|---|
| 手腕贴合画面（关过滤 / 放松参数） | 穿躯帧回来，`robot_sim.mp4` 里手臂埋进身体 —— A1 花了一轮标定才压下去的东西 |
| 不穿躯（保持现在的过滤） | 合成出来的机器人手和画面里人的手差 2–6 cm，**贴上去就能看出来对不齐** |

**要你拍的就是这个取舍**，可选项：① 保持现状（画面错位，几何干净）；
② 合成时用不开碰撞过滤的那份轨迹（画面贴合，可能有穿模帧）；
③ 只对"过滤动过手的帧"降权/标记，导出时写进逐帧质量字段，让下游自己选；
④ 回到 A1 那条没做完的路（检测用接近真身的盒、推出目标用标定盒），指望它把推出量压小。
**我的倾向是 ③ + ④**：③ 不丢信息也不替下游做决定，④ 是本来就欠着的活。

**我没做的事**：没有为了让这三只手的数字好看去改任何过滤参数。参数改动会让
`docs/VERIFICATION.md` 里 ⑤ 那一档的逐位基线和 A1 的 13 段对照表全部失效，
按规矩（「发现但没敢改的存疑数值/设计，记录进 BACKLOG，不要自行决定改动」）记在这。
