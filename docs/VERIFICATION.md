...
## ⑦视觉合成 · 按片段相机渲机器人（`src/web2robot/synth/render.py`，2026-08-23）

同样是新代码，没有"逐位相同"可比；但这一档**动了两个共享点**（`synth/cli.py` 改成
`mask`/`render` 两个子命令、`M7Env.__init__` 多一个 `model=` 入口），所以"没破坏现有
行为"这一半是**逐字节**证的。

```bash
envs/rt_env/bin/python -m unittest tests.test_synth_render -v          # 21 个用例，约 40 秒
bash scripts/dev/check_synth_render_bytes.sh \
     > outputs/dev/synth_render_bytecheck.log 2>&1                     # 掩码那条路一字节没变
scripts/s6_robot_render.sh data/clips_official \
     --runs_dir outputs/retarget/collcmp --pattern '*_grid' \
     --out outputs/synth/render --npz                                  # 8 段，约 6 分钟
```

**没破坏现有行为**：`check_synth_render_bytes.sh` 拿 `git archive HEAD` 抽出旧 `src/`，
用**旧用法（无子命令）**跑一遍 10 段掩码，再用新的 `mask` 子命令跑一遍 —— 20 个
PNG/NPZ 逐字节相同，`handmask.jsonl` 归一化 `--out` 前缀后逐字节相同。掩码这条路
不碰 GPU 不碰随机数，本来就该逐位确定，所以这里要的是最严那种一致（对比
`check_quality_switch_bytes.sh`：那边默认档要跑 KeypointRCNN，只能比判决字段）。

改动面也一起报：`git ls-files` 里的 `.py`/`.sh`/`.yaml`/`.xml` 逐个与 `HEAD` 比 md5，
变了的**只有 3 个** —— `synth/cli.py`、`robots/m7/env.py`、`scripts/s5_hand_mask.sh`，
正好就是上面说的那两个共享点加一个薄壳（命令行用法没变）。其余全是新增文件。

**判据怎么拆**：错位分三个数报，因为 B13 已经证明官方 3D 手和官方 `hand_joints_2d.bin`
自己就差 9.3 px 中位 —— 只报"机器人 ↔ 官方 2D"会把那笔账算到我方链路上。

| 数 | 是谁的账 |
|---|---|
| `robot_mano_px` | **我方链路**（IK + 碰撞过滤 + 相机对齐）|
| `mano_2d_px` | 官方数据自相矛盾（B13），我们改不了 |
| `robot_2d_px` | 两者叠加 = 画面上实际看到的错位 |

**实测**（`outputs/synth/render/render.jsonl`，8 段有 `_grid` 产物的官方片段；
px = `robot_mano_px` 中位，cm = 同一只手的 3D 偏移中位）：

| 片段 | 帧 | 左 px/cm | 右 px/cm | `mano_2d_px` 左/右 | 机器人占画面 | fovy |
|---|---|---|---|---|---|---|
| `--oo8_XIuOM_799.5_809.8` | 155 | 1.94 / 0.64 | 1.28 / 0.39 | 13.0 / 14.5 | 11.3% | 34.54° |
| `--oo8_XIuOM_900.3_917.4` | 257 | 0.58 / 0.48 | **28.86 / 8.89** | 7.4 / 18.6 | 17.8% | 29.05° |
| `-0RheyDV3a0_48.6_55.3` | 104 | 1.01 / 0.25 | 1.86 / 0.41 | 8.9 / 5.2 | 19.1% | 23.90° |
| `-1r9yl-P-Ao_231.8_241.5` | 146 | 1.34 / 0.35 | 0.38 / 0.15 | 5.0 / 4.6 | 16.2% | 27.86° |
| `-1r9yl-P-Ao_60.4_68.4` | 120 | **7.26 / 2.17** | 1.21 / 0.22 | 4.1 / 2.8 | 15.3% | 30.82° |
| `-1r9yl-P-Ao_86.3_90.8` | 69 | 1.64 / 0.31 | **22.67 / 5.19** | 16.6 / 14.5 | 23.0% | 27.89° |
| `-20k07PjLTA_48.0_52.4` | 67 | 1.80 / 0.32 | 1.61 / 0.30 | **89.0** / 26.2 | 22.4% | 40.36° |
| `-2cNMO9Mm3Q_192.4_209.2` | 146 | **31.56 / 5.86** | **14.16 / 2.71** | 7.9 / 6.8 | 15.1% | 29.14° |

`robot_mano_px` 中位 **1.72 px**，16 只手里 11 只 < 2 px。**加粗的 5 只逐只归因过**
（这一步很重要 —— 不归因就没法区分"我方几何错了"和"这一段素材本来就坏"）：

| 手 | 归因依据（`metrics.npz`） | 结论 |
|---|---|---|
| `--oo8_XIuOM_900.3_917.4` 右 | `ik_rate_r` 0.696，`pos_err_r` 最大 0.677 m | **IK 没解出来**（源头坏帧，与 A1 里"那段肉眼可见地坏"是同一段） |
| `-1r9yl-P-Ao_60.4_68.4` 左 | `ik_rate_l` 0.925，个别帧 `pos_err_l` 0.101 m | **IK 个别尖峰帧** |
| `-1r9yl-P-Ao_86.3_90.8` 右 | `ik_rate` 1.0、`pos_err` ≤ 0.01 m；**关掉碰撞过滤重跑 → 22.67 px 掉到 1.02 px** | **碰撞过滤推走的**（BACKLOG B15） |
| `-2cNMO9Mm3Q_192.4_209.2` 左/右 | `ik_rate_l` 1.0 / `ik_rate_r` 0.986，`pos_err` ≤ 0.01 m；**关掉重跑 → 31.56 / 14.16 px 掉到 0.58 / 0.68 px** | 同上（B15） |

后两行的"关掉重跑"是这一档唯一一处**做了对照实验**的归因，因为从产物里看不出来：
`ik_rate` 是 1.0、`pos_err` 是 1 cm 以内，IK 没有失败，是**解完之后的过滤器**改了关节角。
grid 求解器不含随机先验，所以同 seed 重跑的 `root_frames.npz` 逐位相同 ——
`scripts/dev/attrib_wrist_offset.py` 先断言这一条，再报差值，不然"只有过滤这一个变量"
不成立。实测过滤让手腕移了 2.76–5.99 cm（中位）、最大 12.82 cm、约 2/3 的帧被动过。

```bash
# 归因用的对照跑（不进主流程，产物在 outputs/dev/nocoll/）
scripts/s4_retarget.sh "$PWD/data/clips_official/-1r9yl-P-Ao_86.3_90.8" --robot m7 \
    --ckpt runs/m7/taskspace_v2/checkpoints/final.pt --seed 0 --root_solver grid \
    --out outputs/dev/nocoll/-1r9yl-P-Ao_86.3_90.8_grid          # 注意：不给碰撞开关
...
**第二条独立判据：两路深度的尺度对不对得上。** 上面那些都是像素平面上的，证不了
"机器人离相机多远"。拿渲出来的机器人最近点深度和官方 `depth.npz` 的 1% 分位比
（官方那份是 uint16 毫米，独立于 `root_frames.npz` 的平移）：

| 片段 | 场景 1% 分位 | 机器人最近点 | 差 |
|---|---|---|---|
| `--oo8_XIuOM_799.5_809.8` | 1.69 m | 1.67 m | 0.02 |
| `-0RheyDV3a0_48.6_55.3` | 1.58 m | 1.56 m | 0.02 |
| `-1r9yl-P-Ao_86.3_90.8` | 1.30 m | 1.29 m | 0.01 |
| `-1r9yl-P-Ao_60.4_68.4` | 1.23 m | 1.30 m | 0.07 |
| `-1r9yl-P-Ao_231.8_241.5` | 1.35 m | 1.52 m | 0.17 |
| `-2cNMO9Mm3Q_192.4_209.2` | 1.46 m | 1.54 m | 0.08 |
| `--oo8_XIuOM_900.3_917.4` | 0.89 m | 1.59 m | **0.70**（IK 失败那段） |
| `-20k07PjLTA_48.0_52.4` | 0.63 m | 0.47 m | **0.16 / 相对最大**（B13 那段，`depth.npz` 本身重建得差） |

6/8 段在 0.17 m 以内，两个例外正好是已经独立判坏的那两段。**这条很关键** ——
按深度合成（谁挡谁）要求两路深度共用一个尺度，如果差一个量级，合成出来的机器人
会整体在物体前面或后面。它同时也间接证了外参平移的尺度没错。

`mano_2d_px` 那一列最差的 `-20k07PjLTA_48.0_52.4`（89 px）正是 B13 里"手势本身 3D/2D
不一致"的那一段 —— 两处判据独立量出同一段坏，互为佐证。

**顺序无关性（这一档最容易静默烂掉的一条）**：写完 `model.cam_pos` 必须补
`mujoco.mj_camlight`，否则第 0 帧从世界原点拍、之后每帧都用上一帧的相机。
量化证据：补之前第 0 帧机器人占画面 0.2208、补之后 0.6659（同一个位姿）。
`tests/test_synth_render.py` 用"逐帧渲 vs 跳着渲结果相同"钉这条，删掉那一行会红 3 个
（做过突变测试）。同一份测试还钉住"解析算出的手腕投影落在渲出来的机器人掩码里"
（20 px 圆盘内 ≥ 0.96），这一条是把相机数学和渲染结果对起来的唯一独立判据。
